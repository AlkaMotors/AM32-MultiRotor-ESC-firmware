/*
 * phaseouts.c
 *
 *  Created on: Apr 22, 2020
 *      Author: Alka
 */

// TBD: adjust non-pwm output to reflect output polarity (crteensy 20210429)

#include "phaseouts.h"
#include "targets.h"


extern char comp_pwm;
extern char prop_brake_active;

#ifdef USE_INVERTED_COM
	#define COM_ON_REG BRR
	#define COM_OFF_REG BSRR
#else
	#define COM_ON_REG BSRR
	#define COM_OFF_REG BRR
#endif

#ifdef USE_INVERTED_PWM
	#define PWM_ON_REG BRR
	#define PWM_OFF_REG BSRR
#else
	#define PWM_ON_REG BSRR
	#define PWM_OFF_REG BRR
#endif

#ifndef PWM_ENABLE_BRIDGE

void proportionalBrake(){  // alternate all channels between braking (ABC LOW) and coasting (ABC float)
	                        // put lower channel into alternate mode and turn upper OFF for each channel
	// turn all PWM channels off for ABC

	LL_GPIO_SetPinMode(PHASE_A_GPIO_PORT_PWM, PHASE_A_GPIO_PWM, LL_GPIO_MODE_OUTPUT);
	PHASE_A_GPIO_PORT_PWM->PWM_OFF_REG = PHASE_A_GPIO_PWM;

	LL_GPIO_SetPinMode(PHASE_B_GPIO_PORT_PWM, PHASE_B_GPIO_PWM, LL_GPIO_MODE_OUTPUT);
	PHASE_B_GPIO_PORT_PWM->PWM_OFF_REG = PHASE_B_GPIO_PWM;

	LL_GPIO_SetPinMode(PHASE_C_GPIO_PORT_PWM, PHASE_C_GPIO_PWM, LL_GPIO_MODE_OUTPUT);
	PHASE_C_GPIO_PORT_PWM->PWM_OFF_REG = PHASE_C_GPIO_PWM;


	// set low channel to PWM, duty cycle will now control braking
	LL_GPIO_SetPinMode(PHASE_A_GPIO_PORT_COM, PHASE_A_GPIO_COM, LL_GPIO_MODE_ALTERNATE);
	LL_GPIO_SetPinMode(PHASE_B_GPIO_PORT_COM, PHASE_B_GPIO_COM, LL_GPIO_MODE_ALTERNATE);
	LL_GPIO_SetPinMode(PHASE_C_GPIO_PORT_COM, PHASE_C_GPIO_COM, LL_GPIO_MODE_ALTERNATE);
}


void phaseBPWM() {
		if(!comp_pwm){            // for future
		LL_GPIO_SetPinMode(PHASE_B_GPIO_PORT_COM, PHASE_B_GPIO_COM, LL_GPIO_MODE_OUTPUT);
		PHASE_B_GPIO_PORT_COM->COM_OFF_REG = PHASE_B_GPIO_COM;
		}else{
		LL_GPIO_SetPinMode(PHASE_B_GPIO_PORT_COM, PHASE_B_GPIO_COM, LL_GPIO_MODE_ALTERNATE);
		}
		LL_GPIO_SetPinMode(PHASE_B_GPIO_PORT_PWM, PHASE_B_GPIO_PWM, LL_GPIO_MODE_ALTERNATE);
}


void phaseBFLOAT() {
		LL_GPIO_SetPinMode(PHASE_B_GPIO_PORT_COM, PHASE_B_GPIO_COM, LL_GPIO_MODE_OUTPUT);
		PHASE_B_GPIO_PORT_COM->COM_OFF_REG = PHASE_B_GPIO_COM;
		LL_GPIO_SetPinMode(PHASE_B_GPIO_PORT_PWM, PHASE_B_GPIO_PWM, LL_GPIO_MODE_OUTPUT);
		PHASE_B_GPIO_PORT_PWM->PWM_OFF_REG = PHASE_B_GPIO_PWM;
	}


void phaseBLOW() {
	        // low mosfet on
		LL_GPIO_SetPinMode(PHASE_B_GPIO_PORT_COM, PHASE_B_GPIO_COM, LL_GPIO_MODE_OUTPUT);
		PHASE_B_GPIO_PORT_COM->COM_ON_REG = PHASE_B_GPIO_COM;
		LL_GPIO_SetPinMode(PHASE_B_GPIO_PORT_PWM, PHASE_B_GPIO_PWM, LL_GPIO_MODE_OUTPUT);
		PHASE_B_GPIO_PORT_PWM->PWM_OFF_REG = PHASE_B_GPIO_PWM;
}



//////////////////////////////PHASE 2//////////////////////////////////////////////////


void phaseCPWM() {
	if (!comp_pwm){
			LL_GPIO_SetPinMode(PHASE_C_GPIO_PORT_COM, PHASE_C_GPIO_COM, LL_GPIO_MODE_OUTPUT);
			PHASE_C_GPIO_PORT_COM->COM_OFF_REG = PHASE_C_GPIO_COM;
		}else{
		LL_GPIO_SetPinMode(PHASE_C_GPIO_PORT_COM, PHASE_C_GPIO_COM, LL_GPIO_MODE_ALTERNATE);
		}
		LL_GPIO_SetPinMode(PHASE_C_GPIO_PORT_PWM, PHASE_C_GPIO_PWM, LL_GPIO_MODE_ALTERNATE);

	}


void phaseCFLOAT() {
	         // floating
		LL_GPIO_SetPinMode(PHASE_C_GPIO_PORT_COM, PHASE_C_GPIO_COM, LL_GPIO_MODE_OUTPUT);
		PHASE_C_GPIO_PORT_COM->COM_OFF_REG = PHASE_C_GPIO_COM;
		LL_GPIO_SetPinMode(PHASE_C_GPIO_PORT_PWM, PHASE_C_GPIO_PWM, LL_GPIO_MODE_OUTPUT);
		PHASE_C_GPIO_PORT_PWM->PWM_OFF_REG = PHASE_C_GPIO_PWM;
	}



void phaseCLOW() {
		LL_GPIO_SetPinMode(PHASE_C_GPIO_PORT_COM, PHASE_C_GPIO_COM, LL_GPIO_MODE_OUTPUT);
		PHASE_C_GPIO_PORT_COM->COM_ON_REG = PHASE_C_GPIO_COM;
		LL_GPIO_SetPinMode(PHASE_C_GPIO_PORT_PWM, PHASE_C_GPIO_PWM, LL_GPIO_MODE_OUTPUT);
		PHASE_C_GPIO_PORT_PWM->PWM_OFF_REG = PHASE_C_GPIO_PWM;
	}



///////////////////////////////////////////////PHASE 3 /////////////////////////////////////////////////

void phaseAPWM() {
		if (!comp_pwm){
			LL_GPIO_SetPinMode(PHASE_A_GPIO_PORT_COM, PHASE_A_GPIO_COM, LL_GPIO_MODE_OUTPUT);
			PHASE_A_GPIO_PORT_COM->COM_OFF_REG = PHASE_A_GPIO_COM;
			}else{
		LL_GPIO_SetPinMode(PHASE_A_GPIO_PORT_COM, PHASE_A_GPIO_COM, LL_GPIO_MODE_ALTERNATE);
			}
		LL_GPIO_SetPinMode(PHASE_A_GPIO_PORT_PWM, PHASE_A_GPIO_PWM, LL_GPIO_MODE_ALTERNATE);
	}

void phaseAFLOAT() {
		LL_GPIO_SetPinMode(PHASE_A_GPIO_PORT_COM, PHASE_A_GPIO_COM, LL_GPIO_MODE_OUTPUT);
		PHASE_A_GPIO_PORT_COM->COM_OFF_REG = PHASE_A_GPIO_COM;
		LL_GPIO_SetPinMode(PHASE_A_GPIO_PORT_PWM, PHASE_A_GPIO_PWM, LL_GPIO_MODE_OUTPUT);
		PHASE_A_GPIO_PORT_PWM->PWM_OFF_REG = PHASE_A_GPIO_PWM;
	}

void phaseALOW() {
		LL_GPIO_SetPinMode(PHASE_A_GPIO_PORT_COM, PHASE_A_GPIO_COM, LL_GPIO_MODE_OUTPUT);
		PHASE_A_GPIO_PORT_COM->COM_ON_REG = PHASE_A_GPIO_COM;
		LL_GPIO_SetPinMode(PHASE_A_GPIO_PORT_PWM, PHASE_A_GPIO_PWM, LL_GPIO_MODE_OUTPUT);
		PHASE_A_GPIO_PORT_PWM->PWM_OFF_REG = PHASE_A_GPIO_PWM;
	}

#else

//////////////////////////////////PHASE 1//////////////////////
void phaseBPWM() {
		if(!comp_pwm){            // for future
		//LL_GPIO_SetPinMode(PHASE_B_GPIO_PORT_COM, PHASE_B_GPIO_COM, LL_GPIO_MODE_OUTPUT);
		//PHASE_B_GPIO_PORT_COM->BRR = PHASE_B_GPIO_COM;
		}else{
		LL_GPIO_SetPinMode(PHASE_B_GPIO_PORT_ENABLE, PHASE_B_GPIO_ENABLE, LL_GPIO_MODE_OUTPUT); // enable on
		PHASE_B_GPIO_PORT_ENABLE->BSRR = PHASE_B_GPIO_ENABLE;
		}
		LL_GPIO_SetPinMode(PHASE_B_GPIO_PORT_PWM, PHASE_B_GPIO_PWM, LL_GPIO_MODE_ALTERNATE);  // high pwm

	}


void phaseBFLOAT() {
	LL_GPIO_SetPinMode(PHASE_B_GPIO_PORT_ENABLE, PHASE_B_GPIO_ENABLE, LL_GPIO_MODE_OUTPUT); // enable off
	PHASE_B_GPIO_PORT_ENABLE->BRR = PHASE_B_GPIO_ENABLE;
		LL_GPIO_SetPinMode(PHASE_B_GPIO_PORT_PWM, PHASE_B_GPIO_PWM, LL_GPIO_MODE_OUTPUT);  // pwm off
		PHASE_B_GPIO_PORT_PWM->BRR = PHASE_B_GPIO_PWM;
	}


void phaseBLOW() {
	        // low mosfet on
	LL_GPIO_SetPinMode(PHASE_B_GPIO_PORT_ENABLE, PHASE_B_GPIO_ENABLE, LL_GPIO_MODE_OUTPUT);  // enable on
	PHASE_B_GPIO_PORT_ENABLE->BSRR = PHASE_B_GPIO_ENABLE;
		LL_GPIO_SetPinMode(PHASE_B_GPIO_PORT_PWM, PHASE_B_GPIO_PWM, LL_GPIO_MODE_OUTPUT);  // pwm off
		PHASE_B_GPIO_PORT_PWM->BRR = PHASE_B_GPIO_PWM;
}



//////////////////////////////PHASE 2//////////////////////////////////////////////////


void phaseCPWM() {
	if (!comp_pwm){
		//	LL_GPIO_SetPinMode(PHASE_C_GPIO_PORT_COM, PHASE_C_GPIO_COM, LL_GPIO_MODE_OUTPUT);
			//PHASE_C_GPIO_PORT_COM->BRR = PHASE_C_GPIO_COM;
		}else{
			LL_GPIO_SetPinMode(PHASE_C_GPIO_PORT_ENABLE, PHASE_C_GPIO_ENABLE, LL_GPIO_MODE_OUTPUT);  // enable on
			PHASE_C_GPIO_PORT_ENABLE->BSRR = PHASE_C_GPIO_ENABLE;
		}
		LL_GPIO_SetPinMode(PHASE_C_GPIO_PORT_PWM, PHASE_C_GPIO_PWM, LL_GPIO_MODE_ALTERNATE);

	}


void phaseCFLOAT() {
	         // floating
	LL_GPIO_SetPinMode(PHASE_C_GPIO_PORT_ENABLE, PHASE_C_GPIO_ENABLE, LL_GPIO_MODE_OUTPUT);  // enable off
				PHASE_C_GPIO_PORT_ENABLE->BRR = PHASE_C_GPIO_ENABLE;
		LL_GPIO_SetPinMode(PHASE_C_GPIO_PORT_PWM, PHASE_C_GPIO_PWM, LL_GPIO_MODE_OUTPUT);
		PHASE_C_GPIO_PORT_PWM->BRR = PHASE_C_GPIO_PWM;
	}



void phaseCLOW() {
	LL_GPIO_SetPinMode(PHASE_C_GPIO_PORT_ENABLE, PHASE_C_GPIO_ENABLE, LL_GPIO_MODE_OUTPUT);  // enable on
		PHASE_C_GPIO_PORT_ENABLE->BSRR = PHASE_C_GPIO_ENABLE;
		LL_GPIO_SetPinMode(PHASE_C_GPIO_PORT_PWM, PHASE_C_GPIO_PWM, LL_GPIO_MODE_OUTPUT);
		PHASE_C_GPIO_PORT_PWM->BRR = PHASE_C_GPIO_PWM;
	}



///////////////////////////////////////////////PHASE 3 /////////////////////////////////////////////////

void phaseAPWM() {
		if (!comp_pwm){
		//	LL_GPIO_SetPinMode(PHASE_A_GPIO_PORT_COM, PHASE_A_GPIO_COM, LL_GPIO_MODE_OUTPUT);
			//PHASE_A_GPIO_PORT_COM->BRR = PHASE_A_GPIO_COM;
			}else{
				LL_GPIO_SetPinMode(PHASE_A_GPIO_PORT_ENABLE, PHASE_A_GPIO_ENABLE, LL_GPIO_MODE_OUTPUT);  // enable on
				PHASE_A_GPIO_PORT_ENABLE->BSRR = PHASE_A_GPIO_ENABLE;
			}
		LL_GPIO_SetPinMode(PHASE_A_GPIO_PORT_PWM, PHASE_A_GPIO_PWM, LL_GPIO_MODE_ALTERNATE);
	}

void phaseAFLOAT() {
		LL_GPIO_SetPinMode(PHASE_A_GPIO_PORT_ENABLE, PHASE_A_GPIO_ENABLE, LL_GPIO_MODE_OUTPUT);  // enable on
		PHASE_A_GPIO_PORT_ENABLE->BRR = PHASE_A_GPIO_ENABLE;
		LL_GPIO_SetPinMode(PHASE_A_GPIO_PORT_PWM, PHASE_A_GPIO_PWM, LL_GPIO_MODE_OUTPUT);
		PHASE_A_GPIO_PORT_PWM->BRR = PHASE_A_GPIO_PWM;
	}

void phaseALOW() {
		LL_GPIO_SetPinMode(PHASE_A_GPIO_PORT_ENABLE, PHASE_A_GPIO_ENABLE, LL_GPIO_MODE_OUTPUT);  // enable on
		PHASE_A_GPIO_PORT_ENABLE->BSRR = PHASE_A_GPIO_ENABLE;
		LL_GPIO_SetPinMode(PHASE_A_GPIO_PORT_PWM, PHASE_A_GPIO_PWM, LL_GPIO_MODE_OUTPUT);
		PHASE_A_GPIO_PORT_PWM->BRR = PHASE_A_GPIO_PWM;
	}



#endif

void allOff(){
	phaseAFLOAT();
	phaseBFLOAT();
	phaseCFLOAT();
}


void  comStep (int newStep){
//TIM14->CNT = 0;
switch(newStep)
{

        case 1:			//A-B
        	phaseAPWM();
        	phaseBLOW();
        	phaseCFLOAT();
        	break;


        case 2:		// C-B
        	phaseAFLOAT();
        	phaseBLOW();
        	phaseCPWM();
        	break;



        case 3:	// C-A
        	phaseALOW();
        	phaseBFLOAT();
        	phaseCPWM();
        	break;


        case 4:// B-A
        	phaseALOW();
        	phaseBPWM();
        	phaseCFLOAT();
        	break;


        case 5:    // B-C
        	phaseAFLOAT();
        	phaseBPWM();
        	phaseCLOW();
        	break;


        case 6:      // A-C
        	phaseAPWM();
        	phaseBFLOAT();
        	phaseCLOW();
        	break;
	}

//stop_time = TIM14->CNT;

}

void fullBrake(){                     // full braking shorting all low sides
	phaseALOW();
	phaseBLOW();
	phaseCLOW();
}


void allpwm(){                        // for stepper_sine
	phaseAPWM();
	phaseBPWM();
	phaseCPWM();
}
